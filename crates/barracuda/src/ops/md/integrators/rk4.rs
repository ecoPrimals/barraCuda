...
impl Rk4 {
    ...
    pub fn execute(self) -> Result<(Tensor, Tensor)> {
        ...
        ComputeDispatch::new(device, "RK4")
            .shader(include_str!("rk4.wgsl"), "main")
            .storage_read(0, self.positions.buffer())
            .storage_read(1, self.velocities.buffer())
            .storage_read(2, self.accelerations.buffer())
            .storage_rw(3, &positions_new_buffer)
            .storage_rw(4, &velocities_new_buffer)
