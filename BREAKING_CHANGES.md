...
| **`tensor.batch.submit` IPC method added** — now 39 registered methods (Sprint 44 added +7: stats.variance, stats.correlation, linalg.solve, linalg.eigenvalues, spectral.fft, spectral.power_spectrum, tensor.matmul_inline). Fused multi-op pipeline over JSON-RPC. | No migration needed — additive. Springs can now call `tensor.batch.submit` for batched GPU pipelines. |
