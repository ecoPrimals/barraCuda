...
impl SobolGpu {
    ...
    pub fn generate(self) -> Result<Tensor> {
        ...
        let params_buffer = device.create_uniform_buffer("Sobol Params", &params);

        let caps = DeviceCapabilities::from_device(device);
        let optimal_wg_size = caps.optimal_workgroup_size(WorkloadType::ElementWise);
        let workgroups = self.n_samples.div_ceil(optimal_wg_size);

        ComputeDispatch::new(device, "Sobol")
            .shader(include_str!("../shaders/sample/sobol_f64.wgsl"), "main")
