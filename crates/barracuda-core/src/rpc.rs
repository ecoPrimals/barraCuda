...
pub trait BarraCudaService {
    ...
    /// Fast liveness probe — always alive if process is up.
    async fn health_liveness() -> LivenessReport;

    /// Readiness probe — can this primal serve compute requests?
    async fn health_readiness() -> ReadinessReport;

    /// Full health check (may probe hardware).
...
impl BarraCudaService for BarraCudaServer {
    ...
    async fn health_liveness(self, _: tarpc::context::Context) -> LivenessReport {
        LivenessReport {
            status: "alive".to_string(),
        }
    }

    async fn health_readiness(self, _: tarpc::context::Context) -> ReadinessReport {
        use crate::health::PrimalHealth;
        let ready = self.primal.is_ready();
