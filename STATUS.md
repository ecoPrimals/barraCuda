...
| **IPC / primal protocol** | A+ | JSON-RPC 2.0 (notification-compliant) + tarpc; Unix socket default + TCP; capability-based discovery; bare semantic `{domain}.{operation}` method names per wateringHole standard via `REGISTERED_METHODS` + `normalize_method()` backward compatibility; coralReef Phase 10 `shader.compile.*` semantic naming; AMD arch support |
...
- P2: Test coverage ~72% line / ~78% function on llvmpipe (80% CI gate blocking; 90% target requires real GPU hardware; 4,052+ tests + 108 doctests passing)
