            ...
            static DF64_RESULT: std::sync::LazyLock<std::result::Result<String, Arc<str>>> =
                std::sync::LazyLock::new(|| {
                    crate::shaders::sovereign::df64_rewrite::rewrite_f64_infix_full(SHADER)
                        ...
                        .map_err(|e| Arc::from(format!("bessel_j0 DF64 rewrite failed: {e}")))
                });
...
impl BesselJ0F64 {
    ...
    fn j0_scalar(x: f64) -> f64 {
        ...
        if ax >= 8.0 {
            ...
            let pv = 2.093887211e-7f64.mul_add(
                z8,
                2.073370639e-6f64.mul_add(
                    -z6,
                    2.734510407e-5f64.mul_add(z4, 1.098628627e-3f64.mul_add(-z2, 1.0)),
                ),
            );

            let qv = (9.349451520e-8_f64 * z).mul_add(
                -z8,
                (7.621095161e-7_f64 * z).mul_add(
                    z6,
                    (6.911147651e-6_f64 * z).mul_add(
                        -z4,
                        (-1.562499995e-2f64).mul_add(z, 1.430488765e-4 * z * z2),
                    ),
                ),
            );
            ...
            let p = 184.9052456f64.mul_add(
                -z5,
                77392.33017f64.mul_add(
                    z4,
                    11214424.18f64.mul_add(
                        -z3,
                        651619640.7f64.mul_add(z2, 13362590354.0f64.mul_add(-z, 57568490574.0)),
                    ),
                ),
            );

            let q = 1.0_f64.mul_add(
                z5,
                267.8532712f64.mul_add(
                    z4,
                    59272.64853f64.mul_add(
                        z3,
                        9494680.718f64.mul_add(z2, 1029532985.0f64.mul_add(z, 57568490411.0)),
                    ),
                ),
            );
