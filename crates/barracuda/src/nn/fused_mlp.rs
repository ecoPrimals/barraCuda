...
pub async fn fused_mlp(
    ...
    if weights.is_empty() {
        return Err(BarracudaError::invalid_input(
            "fused_mlp: at least one layer required",
        ));
    }
    ...
    if shape.len() != 2 {
        return Err(BarracudaError::invalid_input(format!(
            "fused_mlp: input must be 2D, got {shape:?}"
        )));
    }
    let batch = shape[0];
    ...
    for (i, (w, b)) in weights.iter().zip(biases.iter()).enumerate() {
        ...
        let (out_features, in_feat) = if w_shape.len() != 2 {
            ...
            return Err(BarracudaError::invalid_input(format!(
                "fused_mlp: weight[{i}] shape {w_shape:?} incompatible"
            )));
        };
        if b.shape() != [out_features] {
            return Err(BarracudaError::invalid_input(format!(
                "fused_mlp: bias[{i}] shape mismatch"
            )));
        }
