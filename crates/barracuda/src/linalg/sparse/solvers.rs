...
pub fn bicgstab_solve_with_config(
    ...
    if a.n_cols != n {
        return Err(BarracudaError::invalid_input(
            "BiCGSTAB requires square matrix",
        ));
    }
    if b.len() != n {
        return Err(BarracudaError::invalid_input(format!(
    ...
    for iter in 0..config.max_iterations {
        ...
        if rho_new.abs() < 1e-14 {
            return Err(BarracudaError::internal("BiCGSTAB breakdown: rho = 0"));
        }
        ...
        let rv = dot(&r_hat, &v);
        if rv.abs() < 1e-14 {
            return Err(BarracudaError::internal("BiCGSTAB breakdown: r_hat·v = 0"));
        }
        alpha = rho / rv;
        ...
        if omega.abs() < 1e-14 {
            return Err(BarracudaError::internal("BiCGSTAB breakdown: omega = 0"));
        }
    }
