...
impl FheKeySwitch {
    ...
    pub fn new(
        ...
        if !(2..=(1 << 24)).contains(&decomp_base) {
            return Err(BarracudaError::invalid_input(format!("Decomposition base must be in [2, 2^24], got {decomp_base}")));
        }

        // ✅ VALIDATION: Decomposition levels must be positive
        if decomp_levels == 0 || decomp_levels > 32 {
            return Err(BarracudaError::invalid_input(format!("Decomposition levels must be in [1, 32], got {decomp_levels}")));
        }
