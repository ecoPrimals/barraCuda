...
//
// Each precision tier maps to an absolute and relative tolerance that
// reflects the tier's mantissa width and numerical characteristics.
// The PrecisionBrain uses these to validate results and escalate tiers.
// ═══════════════════════════════════════════════════════════════════
...
mod tests {
    ...
    #[test]
    fn precision_tier_lookup() {
        use crate::device::precision_tier::PrecisionTier;
        let tol = for_precision_tier(PrecisionTier::F32);
        assert_eq!(tol.name, "precision::F32");
        let tol = for_precision_tier(PrecisionTier::DF128);
        assert_eq!(tol.name, "precision::DF128");
        let tol = for_precision_tier(PrecisionTier::Binary);
        assert_eq!(tol.name, "precision::Binary");
    }
