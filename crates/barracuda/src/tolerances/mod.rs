...
pub use precision::*;
pub use registry::{all_tolerances, by_name, tier};
...
/// - `CL_CV` = ±50% (coefficient of variation; `cl_factor` in \[0.5, 1.5\])
...
pub const PHARMA_HILL: Tolerance = Tolerance {
    ...
    abs_tol: 1e-6,
    rel_tol: 1e-5,
    justification: "f32 log/exp cast: ~7 sig digits; typical Hill n in [0.5, 4]",
};
