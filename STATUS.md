...
| **Sovereign compiler** | A+ | FMA fusion + dead expr elimination + safe WGSL roundtrip (all backends); sovereign validation harness covers all 816 shaders via rayon parallel validation; `erfc_f64` recursion eliminated; **NagaExecutor** CPU interpreter for naga IR (f32+f64 native, shared memory, barriers, atomics); coralReef CPU compilation IPC contract (`shader.compile.cpu`, `shader.execute.cpu`, `shader.validate`) |
...
- P2: Test coverage ~72.83% line on llvmpipe (80% CI gate blocking; 90% target requires real GPU hardware; 4,150+ tests + 108 doctests)
