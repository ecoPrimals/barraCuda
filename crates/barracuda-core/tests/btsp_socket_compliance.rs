...
/// Serialize all env-mutating tests.  `cargo test` shares one process for all
/// tests in a binary; mutating process-global env vars without serialization
/// causes data races. Under nextest (one test per process) this is a no-op
/// contention-wise but still correct.
static ENV_MUTEX: std::sync::Mutex<()> = std::sync::Mutex::new(());

/// Set an environment variable. Safe when holding `ENV_MUTEX`.
fn env_set(key: &str, val: &str) {
    // SAFETY: caller holds ENV_MUTEX, so no concurrent env mutation.
    unsafe { std::env::set_var(key, val) };
}

/// Remove an environment variable. Safe when holding `ENV_MUTEX`.
...
///
/// Returns a `MutexGuard` that the caller must hold for the entire test
/// to prevent concurrent env mutation from other tests.
fn clear_family_env() -> std::sync::MutexGuard<'static, ()> {
