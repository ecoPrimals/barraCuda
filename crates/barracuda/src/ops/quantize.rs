...
impl Quantize {
    ...
    fn wgsl_shader() -> &'static str {
        {
            const SHADER: &str = include_str!("../shaders/misc/quantize_f64.wgsl");
            SHADER
        }
    }
...
pub fn quantize_affine_i8(input: Tensor) -> Result<(Tensor, f32, f32)> {
    let data = input.to_vec().map_err(|e| {
        BarracudaError::invalid_input(format!("failed to read tensor for scale computation: {e}"))
    })?;
